...
echo > $HOME/repos/dotfiles/i3/config && # empty the current config file
  for f in $HOME/repos/dotfiles/i3/i3.*; do (
    cat "${f}"
    echo
  ) >> $HOME/repos/dotfiles/i3/config; done && # cat every file and add a new line every file and append everything to the config file
  sendMessages=${1:-true}
